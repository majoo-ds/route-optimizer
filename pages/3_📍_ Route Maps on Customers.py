...
st.markdown("# Route Optimizer on Majoo's Existing Merchants")
    ...
    if post_ors_api:
        try:
            ...
            if result:
                ...
                df_merged_clean["google_maps_url"] = df_merged_clean["google_maps"].apply(lambda x: f'<a href="{x}">{x}</a>') # for showing
